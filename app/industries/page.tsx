    ...
    <main className="section-shell bg-white py-16">
      <div className="container-shell">
        <section className="py-10 text-center">
          <h1 className="text-4xl font-extrabold text-[#0A0A0A] sm:text-6xl">Who We Help</h1>
          <p className="mt-3 text-[#6B7280]">We&apos;ve worked with teams across industries.</p>
        </section>
        ...
        <section className="grid gap-6 pb-12 md:grid-cols-2 lg:grid-cols-3">
          {industries.map((industry, index) => {
            ...
            return (
              <article key={industry.name} className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-[var(--shadow-sm)]">
                ...
                <div className="mt-4">
                  <p className="text-sm font-semibold text-[#0A0A0A]">Challenges we solve:</p>
                  <ul className="mt-2 space-y-2">
                    {industry.challenges.map((challenge) => (
                      <li key={challenge} className="flex items-center gap-2 text-sm text-[#6B7280]">
